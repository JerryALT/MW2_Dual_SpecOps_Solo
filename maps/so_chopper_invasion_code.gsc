...
debug_chopper_target_entity(ent)
{
	color = ( 1, 0, 1 );
	/#
	while(isDefined(ent) && isAI(ent) && isAlive(ent) && self canSee(ent))
	{
...
chopper_minigun_spawn_think()
{
    level.turret = spawnturret( "misc_turret", (0,0,0), "heli_minigun_noai" );
    level.turret setmodel( "weapon_saw_MG_setup" );

    level.turret linkto( self, "tag_turret_base", ( 0, 0, 10 ), ( 0, 0, 0 ) );

	/#
	level.turret thread maps\_debug::drawOrgForever();
	#/
	thread so_fake_choppergunner();
	level.turret makeUnusable();
    level.turret setmode( "manual" );
    level.turret setturretteam( "allies" );
	...
	/#
	iPrintLnBold("turret start shoot");
	#/

	level.turret thread chopper_ai_minigun_think();
}

waittill_death()
{
	level endon ("chopper_target_reset");

	self waittill( "death", attacker );
	
	if ( ( isdefined( attacker ) ) && ( attacker != level.player ) )
	{
		level.player play_sound_on_tag ("SO_US_2_inform_kill_generic");
	}
	wait randomfloat(5, 10);
}

chopper_ai_minigun_think(target_reaquire_time)
{	
	level endon ("special_op_terminated");

    while( true )
	{
		if(!isDefined(target_reaquire_time))
			target_reaquire_time = randomFloatRange(3,6);

        enemies = getaiarray( "axis" );
        if( enemies.size > 0 )
        {
			//target = enemies[ randomint( enemies.size ) ];
			target = getClosest( level.player.origin, enemies );
            if ( Distance( target.origin, level.player.origin ) >= 350 )
			{
				if(self canSee(target))
				{
					level.miniguns_firing = true;
					self thread chopper_minigun_shoot(target);
					self thread debug_chopper_target_entity(target);
					self setTargetEntity( target );
					target waittill_death();
					self clearTargetEnt();
					level.miniguns_firing = false;
					level notify("chopper_target_reset");
				}
				else
				{
					level notify("chopper_target_reset");
		...
		{
			level notify("choppergunner_no_targets");
			level.miniguns_firing = false;
		}
...
chopper_minigun_shoot(ent) 
{
	level endon ("choppergunner_no_targets");
	level endon ("chopper_target_reset");
	level endon ("special_op_terminated");
	...
	if ( level.miniguns_firing )
    {
		while(isAlive(ent))
		{
			...
			wait randomfloat( 3, 6 );
		}
    }
}

#using_animtree( "generic_human" );
choppergunner_friendlyfire()
{
	level endon ("special_op_terminated");
	...
	if ( ( isdefined( attacker ) ) && ( attacker == level.player ) )
	{
		iPrintLnBold("friendly fire!");
		level.player play_sound_on_tag ("SO_US_2_threat_friendly_fire");
		wait randomfloat(5, 10);
	}
	if(amount <= self.script_health)
	{
...
choppergunner_think()
{
	...
	self character\character_us_army_assault_a::main();
	self useanimtree( #animtree );
	self.script_health = 150;
	self thread choppergunner_friendlyfire(); // doesn't works with script_model :(
}

so_fake_choppergunner()
{
	drone = spawn( "script_model", (0,0,0) );
	drone choppergunner_think();

	drone LinkTo( level.chopper, "tag_turret_base", ( -25,0,-18 ), ( 0,0,0 ) );

	level.scr_anim[ "generic" ][ "crouch_shoot" ] = %exposed_crouch_aim_5;
	drone setanimknob( level.scr_anim[ "generic" ][ "crouch_shoot" ], 1, 0, 0 );
}
...
debug_player_radius()
{
	...
	while(1)
	{
		thread player_radius_checking(1, level.player getOrigin(), 350);
		wait 0.05;
	}
	#/
}

player_radius_checking( time, org, radius )
{
	/#
	for ( i = 0;i < time * 20;i++ )
	{
