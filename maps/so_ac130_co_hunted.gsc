...
start_ac130_support()
{
    map_center_origin = spawn( "script_origin", ( 0, 0, 0 ) );
    map_center_origin thread spawn_ac130( -16000, 6000 );
}

spawn_ac130( width, height )
{
    ac130_origin = spawn( "script_origin", self.origin );
    ac130_origin.origin = self.origin + ( 0, 0, height );
    ac130_origin.angles = ( 0, 0, 0 );

    level.ac130_model = spawn( "script_model", self.origin );
    level.ac130_model.origin = self.origin;
    level.ac130_model.angles = self.angles;
    level.ac130_model SetModel( "vehicle_ac130_coop" );
	level.ac130_model playLoopSound( "veh_ac130_ext_dist" );
    level.ac130_model LinkTo( ac130_origin, "", ( width, 0, height ), ( 45, 0, 0 ) );
	level.ac130_model thread fireAC130WeaponsOnEnemies();

    while ( 1 )
    {
		/#
		level.ac130_model thread maps\_debug::drawOrgForever();
		#/
        ac130_origin RotateYaw( 360, 180 );
        wait( 180 );
    }
}

fireAC130WeaponsOnEnemies()
{
	level endon ("special_op_terminated");
	...
	/#
	iprintlnbold("specops_start");
	#/

	wait randomFloatRange(3,6);

	/#
	iprintlnbold("AC130_logic_on");
	#/
	...
	while ( 1 )
	{
		...
		if ( enemies.size > 0 )
		{
			val = randomIntRange( 1, 5 );
			if ( !isDefined( self.fired105mm_cannon ) && val == 4 )
			{
				self thread fire_ac130_105mm_weapon();

				self waittill( "fired105mm_cannon" );

				self.fired25mm_cannon = undefined;
				self.fired40mm_cannon = undefined;
			}
			else if ( !isDefined( self.fired40mm_cannon ) && val == 3 )
			{
				self thread fire_ac130_40mm_weapon();

				self waittill( "fired40mm_cannon" );

				self.fired25mm_cannon = undefined;
				self.fired105mm_cannon = undefined;
			}
			else if ( !isDefined( self.fired25mm_cannon ) )
			{
				self thread fire_ac130_25mm_weapon();

				self waittill( "fired25mm_cannon" );
...
fire_ac130_105mm_weapon()
{
	while ( 1 )
	{
		enemies = GetAIArray( "axis" );
		farthestEnemy = getFarthest( level.player.origin, enemies );
		val = randomIntRange( 1, 5 );
		if ( enemies.size < 1 || val > 2 || Distance( farthestEnemy.origin, level.player.origin ) <= 600 /*|| !self AC130CanSeeEnemy( farthestEnemy ) */)
		{
			self.fired105mm_cannon = true;
			break;
		}

		magicbullet( "ac130_105mm", self.origin, farthestEnemy.origin );
		wait( randomFloatRange( 3, 4 ) );
	}

	wait( 0.05 );

	self notify( "fired105mm_cannon" );
}

fire_ac130_40mm_weapon()
{
	while ( 1 )
	{
		enemies = GetAIArray( "axis" );
		farthestEnemy = getFarthest( level.player.origin, enemies );
		val = randomIntRange( 1, 10 );
		if ( enemies.size < 1 || val > 6 || Distance( farthestEnemy.origin, level.player.origin ) <= 400 /*|| !self AC130CanSeeEnemy( farthestEnemy ) */)
		{
			self.fired40mm_cannon = true;
			break;
		}

		magicbullet( "ac130_40mm", self.origin, farthestEnemy.origin );
		wait( randomFloatRange( 1, 1.3 ) );
	}

	wait( 0.05 );

	self notify( "fired40mm_cannon" );
}

fire_ac130_25mm_weapon()
{
	while ( 1 )
	{
		enemies = GetAIArray( "axis" );
		farthestEnemy = getFarthest( level.player.origin, enemies );
		val = randomIntRange( 1, 40 );
		if ( enemies.size < 1 || val > 34 || Distance( farthestEnemy.origin, level.player.origin ) <= 250 /*|| !self AC130CanSeeEnemy( farthestEnemy ) */)
		{
			self.fired25mm_cannon = true;
			break;
		}

		magicbullet( "ac130_25mm", self.origin, farthestEnemy.origin );
		wait( randomFloatRange( 0.1, 0.3 ) );
	}

	wait( 0.05 );

	self notify( "fired25mm_cannon" );
}

AC130CanSeeEnemy( enemy )
{
	if ( SightTracePassed( self.origin - ( 0, 0, 300 ), enemy.origin + ( 0, 0, 150 ), false, undefined ) )
		return true;

	trace = bullettrace( ( self.origin - ( 0, 0, 150 ) ), ( enemy.origin + ( 0, 0, 150 ) ), 0, undefined );
	groundtype = trace[ "surfacetype" ];
	if ( !isdefined( groundtype ) || groundtype == "metal" || groundtype == "paintedmetal" )
		return true;
